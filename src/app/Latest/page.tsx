...
export default function LatestNews() {
  const timelineRef = useRef<HTMLDivElement>(null); // Explicitly set the type as HTMLDivElement
  const [lineHeight, setLineHeight] = useState(0);

  // Update the line height dynamically based on content height
  useEffect(() => {
    if (timelineRef.current) {
      const totalHeight = timelineRef.current.scrollHeight; // Now TypeScript knows scrollHeight exists
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-600 via-indigo-500 to-purple-600 overflow-hidden">
      {/* Header Section */}
      <div className="w-full py-8 sticky top-0 bg-[#1e293b] z-10"> {/* Added sticky and z-index */}
        <div className="ml-8">
      ...
      {/* Timeline Section */}
      <div ref={timelineRef} className="relative container mx-auto my-8 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 12rem)', scrollBehavior: 'smooth' }}> {/* Smooth scrolling */}
        {/* Vertical Line */}
        <div
          className="absolute w-[6px] bg-gray-300 left-[50%] transform -translate-x-[50%]"
          style={{ height: `${lineHeight}px` }} // Dynamic height based on content
        ></div>
        ...
        {newsItems.map((item, index) => (
          ...
          >
            ...
            <div
              className={`w-[40%] ${index % 2 === 0 ? "text-left ml-12" : "text-left mr-12"}`}
            >
              <div className="bg-[#334155] p-6 rounded-lg shadow-lg relative">
                <h3 className="mb-3 font-bold text-white text-xl">
                  {item.title}
                </h3>
                {/* Date below the heading */}
            ...
            <div
              className={`z-20 flex items-center justify-center w-[150px] h-[150px] rounded-full bg-blue-600 shadow-xl border border-white ${index % 2 === 0 ? "ml-auto" : "mr-auto"}`}
            >
